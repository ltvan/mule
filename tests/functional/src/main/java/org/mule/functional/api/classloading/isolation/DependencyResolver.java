 ...
 * build class loaders by filtering an initial and complete classpath urls and using a maven dependency graph represented by their
...
public class DependencyResolver {
  ...
  /**
   * Builds a list of {@link MavenArtifact} representing the transitive dependencies of the provided dependency
   * {@link MavenArtifact}.
   *
   * @param dependency a {@link MavenArtifact} for which we want to know its dependencies.
   * @param predicate a filter to be applied for each transitive dependency, if the filter passes the dependency is added and
   *        recursively collected its dependencies using the same filter.
  ...
  private Set<MavenArtifact> collectTransitiveDependencies(final MavenArtifact dependency,
    ...
    configuration.getDependencyGraph().getTransitiveDependencies(dependency).stream().forEach(transitiveDependency -> {
      if (predicate.test(transitiveDependency)) {
        ...
        // Just the case for getting all their dependencies from an excluded dependencies (case of org.mule:core for instance, we
        // also need their transitive dependencies)
